...
predict2_video2world_lora_training_2b_custom_data = dict(
    ...
    job=dict(
        ...
        name="2b_my_custom_data_mix",  # Name the experiment --> affect the output directory path
    ),
    model=dict(
        config=dict(
    ...
    trainer=dict(
        ...
        max_iter=30000,                      # LoRA 训练迭代次数
    ),
    checkpoint=dict(
        save_iter=5000,                      # 2k 保存一次
    ),
    optimizer=dict(
        lr=2 ** (-10)
        # lr = 1.5e-4                      # LoRA 使用较高的学习率
    ),
    scheduler=dict(        # 老师 
        warm_up_steps=[1500],
        cycle_lengths=[30_000],     # 最大迭代数
        f_max=[1.0],
        f_min=[0.05],
    ),
)
