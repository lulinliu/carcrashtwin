...
custom_video_lora_dataset = L(Dataset)(
    dataset_dir="/scratch/10102/hh29499/carcrashtwin/egotl/output_clips_final_Nov10_1am",  # <--- 修改这里！改成你的数据集路径
    num_frames=93,                                          # 视频帧数
...
predict2_video2world_lora_training_2b_custom_data = dict(
    ...
    job=dict(
        ...
        name="2b_custom_data_egotl_10k",  # Name the experiment --> affect the output directory path
    ),
    model=dict(
        config=dict(
...
for _item in [
    ...
    cs.store(
        group="experiment",
        package="_global_",
        name="predict2_video2world_lora_training_2b_nov11_5k",
        node=_item,
    )
