...
custom_video_lora_dataset = L(Dataset)(
    dataset_dir="crash_cosmos/",  # <--- 修改这里！改成你的数据集路径
    num_frames=93,                                          # 视频帧数
...
predict2_video2world_lora_training_14b_custom_data = dict(
    ...
    job=dict(
        ...
        name="14b_custom_data_crash",  # Name the experiment --> affect the output directory path
    ),
    model=dict(
        config=dict(
...
for _item in [
    ...
    cs.store(
        group="experiment",
        package="_global_",
        name="predict2_video2world_lora_training_14b_realnuo_14b_1117",
        node=_item,
    )
